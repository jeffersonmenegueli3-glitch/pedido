...
import { RefreshCw, Search, Bell, Mail, Truck, ShieldCheck, LogOut, CheckCircle, MessageSquare, Sun, Moon, MapPin } from 'lucide-react';
  ...
  return (
    <header className="bg-slate-900 border-b border-slate-800 sticky top-0 z-30 px-4 lg:px-8 py-3 shadow-md">
      <div className="flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        {/* Global Search Input & Unidade Filter */}
        <div className="flex flex-col sm:flex-row items-center gap-2 w-full md:w-auto">
          ...
          {/* Unidade / Operação Selector */}
          {onSelectOperacao && (
            <div className="flex items-center gap-1 p-1 bg-slate-950/80 border border-slate-800 rounded-xl text-xs font-bold shadow-inner w-full sm:w-auto justify-center">
              <div className="flex items-center gap-1 px-1.5 text-slate-400 font-bold text-[11px] shrink-0">
                <MapPin className="w-3.5 h-3.5 text-indigo-400" />
                <span className="hidden xl:inline">Unidade:</span>
              </div>
              {['Todas', 'Rio', 'Interior', 'Redespacho'].map((op) => {
                ...
                return (
                  <button
                    ...
                    title={`Mudar orçamento e dados para a unidade: ${op}`}
                  >
                    {op === 'Todas' ? 'Todas' : op}
                  </button>
                );
